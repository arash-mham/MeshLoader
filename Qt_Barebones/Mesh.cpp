...
while( true ){
	...
	{
		unsigned short vertexIndexX;
		unsigned short vertexIndexY;
		unsigned short vertexIndexZ;
		int matches = fscanf(file, "%hu %hu %hu\n", &vertexIndexX, &vertexIndexY,&vertexIndexZ);
		index_list.push_back(vertexIndexX);
		index_list.push_back(vertexIndexY);
		index_list.push_back(vertexIndexZ);
	}
...
Vertex* stackVerts;
	...
	for(int i=0;i<vertex_list.size();i++)
		stackVerts[i]=vertex_list[i];
	
	ret.numVertices = vertex_list.size();
	ret.vertices = new Vertex[ret.numVertices];
	memcpy(ret.vertices, stackVerts,vertex_list.size()*sizeof(Vertex));

	unsigned short* stackIndices;
	stackIndices=new unsigned short[index_list.size()];
	for(int i=0;i<index_list.size();i++)
		stackIndices[i]=index_list[i]-1;
	ret.numIndices=index_list.size();
		
	ret.numIndices=index_list.size();
	ret.indices = new GLushort[ret.numIndices];
	memcpy(ret.indices, stackIndices, index_list.size()*sizeof(GLushort));
	//delete[]stackVerts;
	//delete[]stackIndices;
